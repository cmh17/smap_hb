...
from data_loader_unet3 import CombinedDataset
from unet4 import UNetFusion

# Same architecture as unet 2, but uses fixed data loader (data loader 3)
...
root_dir = f"{workspace}/data/combined_output/tiles3"

loading_start_time = time.time()

ds = xr.open_dataset(os.path.join(workspace, "data", "combined_output", "tiles3", "tile_0_0", "dynamic.nc"))
...
training_set = CombinedDataset(root_dir, time_inds=train_time_inds, listofstatic=static_vars)
validation_set = CombinedDataset(root_dir, time_inds=test_time_inds, listofstatic=static_vars)
...
for epoch in range(num_epochs):
    ...
    if avg_vloss < best_vloss:
        ...
        model_path = f"{workspace}/scripts/model_outputs/unet4_std_model_{timestamp}_epoch{epoch}.pth"
...
plt.savefig(f"{workspace}/scripts/model_outputs/unet4_std_training_validation_loss_{timestamp}.png", dpi=300)
